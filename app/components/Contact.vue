      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-1">
          ...
          <div v-else class="space-y-4">
            <a v-for="method in displayContactMethods" :key="method.title" :href="method.link" target="_blank" rel="noopener" class="card p-4 hover:border-blue-500 block transition">
        ...
        <div class="lg:col-span-2">
          <form @submit="handleSubmit" class="space-y-4">
            <transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-200" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-2">
              <div v-if="submitted" class="p-4 border rounded-lg text-sm" :style="{ backgroundColor: 'var(--selection)', borderColor: 'var(--success)', color: 'var(--success)' }">
                {{ submitMessage }}
              </div>
            </transition>

            <transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-200" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-2">
              <div v-if="submitMessage.includes('Failed')" class="p-4 border rounded-lg text-sm" :style="{ backgroundColor: '#3a2a2a', borderColor: 'var(--danger)', color: 'var(--danger)' }">
                {{ submitMessage }}
              </div>
            </transition>

            <div>
              <label for="name2" class="text-success block text-sm font-medium mb-2">name</label>
              <input id="name2" v-model="form.name" type="text" placeholder="John Doe"
            ...
            <div>
              <label for="email2" class="text-success block text-sm font-medium mb-2">email</label>
              <input id="email2" v-model="form.email" type="email" placeholder="john@example.com"
                class="w-full px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition"
            ...
            <div>
              <label for="subject2" class="text-success block text-sm font-medium mb-2">subject</label>
              <input id="subject2" v-model="form.subject" type="text" placeholder="Project Inquiry"
                class="w-full px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition"
            ...
            <div>
              <label for="message2" class="text-success block text-sm font-medium mb-2">message</label>
              <textarea id="message2" v-model="form.message" rows="6" placeholder="Tell me about your project..."
                ...
                :style="{ backgroundColor: 'var(--bg)', border: '1px solid var(--border)', color: 'var(--text-primary)' }"></textarea>
            ...
            <div class="pt-4">
              <button type="submit" :disabled="sending" class="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
