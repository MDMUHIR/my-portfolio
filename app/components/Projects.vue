...
const defaultProjects = [
  {
    ...
    longDescription:
      "A full-stack application featuring intelligent search capabilities.",
    category: "AI & Web",
    tags: ["AI", "Node", "Vue", "ML"],
    image: "🤖",
    link: "#",
    github: "#",
    highlights: [
      "Real-time search with ML recommendations",
  ...
  },
  {
    ...
    description:
      "Collaborative project management tool with real-time updates and Kanban boards.",
...
const displayProjects = computed(() =>
  projects.value.length > 0 ? projects.value : defaultProjects,
);
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6">
      ...
      >
        <NuxtLink
          ...
          class="card overflow-hidden group transition-all duration-300 hover:-translate-y-1 reveal block"
          :class="[`stagger-${(idx % 6) + 1}`]"
        >
          <div
            class="p-6 sm:p-6 flex items-center justify-center min-h-28 sm:min-h-32 transition-all duration-300 group-hover:scale-105 overflow-hidden"
            :style="{ background: 'var(--gradient-card)' }"
          >
            <img
              v-if="isImageUrl(p.image)"
              :src="p.image"
              class="w-full h-full object-cover rounded-lg"
            />
            <div
              v-else
              class="text-5xl sm:text-6xl transition-transform duration-300 group-hover:scale-110"
            >
              {{ p.image }}
            </div>
          </div>
          ...
          <div class="p-5 sm:p-6">
            <div class="mb-3">
              <h3
                class="text-theme font-bold text-base sm:text-lg group-hover:text-accent-light transition-colors duration-200"
              >
                {{ p.title }}
              </h3>
              <p class="text-accent text-xs font-semibold mt-0.5">
                {{ p.category }}
              </p>
            </div>

            <p
              class="text-theme-secondary text-xs sm:text-sm mb-4 line-clamp-2 leading-relaxed"
            >
              {{ p.description }}
            </p>

            <div
              v-if="p.stats && Object.keys(p.stats).length"
              class="grid grid-cols-3 gap-2 mb-4 p-3 rounded-lg text-xs"
              :style="{
            ...
            >
              <div
                ...
                class="text-center"
              >
                <div class="text-accent-light font-bold text-sm">
                  {{ value }}
                </div>
                <div class="text-theme-secondary capitalize truncate">
            ...
            <div
              v-if="p.highlights && p.highlights.length"
              class="mb-4 space-y-1"
            >
              <div
                v-for="(h, hi) in p.highlights.slice(0, 3)"
                :key="hi"
                class="flex items-center gap-1.5 text-xs text-theme-secondary"
              >
