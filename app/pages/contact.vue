...
<template>
  <div class="py-10 sm:py-12 section-alt min-h-[calc(100vh-4rem)]">
    <div class="max-w-5xl mx-auto px-4 sm:px-6">
      <h1 class="section-title mb-2">// Get in Touch</h1>
      <p class="text-theme-secondary text-sm sm:text-base mb-8 sm:mb-10">Have a project in mind? Let's build something great together.</p>
      ...
      <div class="flex flex-col lg:grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 order-1 lg:order-2">
          <form @submit="handleSubmit" class="space-y-4 sm:space-y-5">
            ...
            <div>
              <label for="contact-name" class="text-success block text-sm font-medium mb-1.5">name</label>
              <input id="contact-name" v-model="form.name" type="text" placeholder="John Doe" class="input-theme" :class="{ 'border-red-500': errors.name }" />
              <p v-if="errors.name" class="text-danger mt-1 text-xs">{{ errors.name }}</p>
            </div>

            <div>
              <label for="contact-email" class="text-success block text-sm font-medium mb-1.5">email</label>
              <input id="contact-email" v-model="form.email" type="email" placeholder="john@example.com" class="input-theme" :class="{ 'border-red-500': errors.email }" />
              <p v-if="errors.email" class="text-danger mt-1 text-xs">{{ errors.email }}</p>
            </div>

            <div>
              <label for="contact-subject" class="text-success block text-sm font-medium mb-1.5">subject</label>
              <input id="contact-subject" v-model="form.subject" type="text" placeholder="Project Inquiry" class="input-theme" :class="{ 'border-red-500': errors.subject }" />
              <p v-if="errors.subject" class="text-danger mt-1 text-xs">{{ errors.subject }}</p>
            </div>

            <div>
              <label for="contact-message" class="text-success block text-sm font-medium mb-1.5">message</label>
              <textarea id="contact-message" v-model="form.message" rows="5" placeholder="Tell me about your project..." class="input-theme resize-none" :class="{ 'border-red-500': errors.message }"></textarea>
